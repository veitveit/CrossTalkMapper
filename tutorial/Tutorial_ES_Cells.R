...
# TODO Tell that timepoint variable needs to be set accordingly
mytable$timepoint <- "None"
mytable$biological.replicate <- mytable$repl <- 1
#ENSURE THE ROW WITH TISSUE INFO HAS HEADER "tissue"
mytable$tissue <- mytable$cell.type...tissue
write.csv(mytable, "t.csv")


# histvars=TRUE is you wish to distinguish between H3.1 and H3.3. Else set to FALSE
...
# TODO create folder "crosstalkmaps
...
for (mi_pos in poi) {
  ...
  for (mi in mis) {
    ...
    ## no filter
    CrossTalkMap(ptm_ab_mi, splitplot_by = "timepoint", colcode = "pj", connected = "tissue", group_by = "repl",
    ...
    for (top in c(5, 10)) {
      ...
      CrossTalkMap(midat_plot, splitplot_by = "timepoint", colcode = "pj", connected = "tissue", group_by = "repl",
                   connect_dots = TRUE, with_arrows = TRUE,
...
pos_combs <- list(c("K9", "K14"), c("K9", "K18"), c("K9", "K23"), c("K9", "K27"), c("K9", "K36"), 
                  c("K14", "K18"), c("K14", "K23"), c("K14", "K27"), c("K14", "K36"),
...
for (pos_comb in pos_combs) {
  ...
  CrossTalkMap(ptm_ab_pos_mod, splitplot_by = "tissue", colcode = "pj", connected = "timepoint", group_by = "repl",
               ...
               filename_string = paste0(paste0(pos_comb, collapse = '-'), "ac"), outdir = "plots/")
